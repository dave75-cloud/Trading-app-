...
def normalize_signal(value):
    """
    Normalize the frozen M006e signal vocabulary to {-1, 0, 1}.

    Accepted semantic encodings:
        long  ->  1
        short -> -1
        flat  ->  0

    Accepted numeric encodings:
        1, 0, -1
        "1", "0", "-1"
    ...
    if isinstance(value, str):
        ...
        try:
            ...
            raise ValueError(
                f"invalid signal encoding: {value!r}"
            )
    else:
        try:
    ...
    if normalized not in (-1, 0, 1):
        raise ValueError(
            f"invalid signal value: {normalized!r}"
        )
...
def validate_entry_leg(
    ...
    authoritative_signal = normalize_signal(
        event[
...
def validate_exit_leg(
    ...
    out["oanda_signal"] = normalize_signal(
        event[
            "desired_delayed_signal"
        ]
    )
    ...
    if twelve_row is None:
        ...
        if twelve_signal != normalize_signal(
            event[
                "desired_delayed_signal"
            ]
        ):
