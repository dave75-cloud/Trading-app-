...
if ! m006e5_acquire_lock; then
    echo "SKIP: M006e.5 lock not acquired"
    exit 0
fi
...
LOCK_HELPER="$PROJECT/tools/m006e/m006e5/stale_lock_candidate/m006e5_stale_lock.sh"
if [ ! -f "$LOCK_HELPER" ]; then
    echo "FAIL_CLOSED: stale-lock helper missing"
    exit 16
fi
. "$LOCK_HELPER" || {
