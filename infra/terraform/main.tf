...
# If create_secrets=true and *_secret_arn is empty, this root module creates secrets.
# Otherwise it uses the provided ARNs.
# -------------------------------------------------------------------
...
module "ecs" {  source = "./ecs"
  ...
  # secrets ARNs for runtime (if your ecs module expects these)
  ...
  # optional ALB logs bucket if your ecs module supports it
...
module "rds" {
  ...
  # wire alarms topic from ECS (must exist as an output in ecs module)
  alarms_topic_arn = module.ecs.alarms_topic_arn
}

EOFcat > infra/terraform/main.tf <<'EOF'
terraform {
  required_version = ">= 1.5.0"
  required_providers {
    aws = {
