...
DEFAULT_EXCLUDE_DAYS = {
    "2025-01-14",  # known partial weekday anomaly
}


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("--input", required=True, help="Path to resampled CSV")
    ap.add_argument("--output", required=True, help="Path to cleaned output CSV")
    ap.add_argument(
        "--exclude-days",
        nargs="*",
        default=None,
        help="Optional explicit list of YYYY-MM-DD days to exclude",
    )
    ap.add_argument(
        "--drop-sunday-reopen",
        action="store_true",
        help="Drop Sunday partial reopen bars",
    )
    ap.add_argument(
        "--min-bars-per-day",
        type=int,
        default=None,
        help="Optional daily row-count threshold; days below this are removed",
    )
    args = ap.parse_args()
    ...
    if args.min_bars_per_day is not None:
        counts = df.groupby("day").size().rename("row_count")
